...
template <typename Complex> struct Callback {
  ...
  int operator()(Complex &F, const std::array<KeyType, k> &new_name,
...
struct complex_traits {
  ...
  using NodeTypes = util::type_holder<int, int, int, int>; /**< @brief the types
                                                              of each Node */
  using EdgeTypes =
...
int main(int argc, char *argv[]) {
  ...
  for (auto vid : mesh.get_level_id<1>()) {
    std::cout << "EXPECT_EQ(mesh.onBoundary(mesh.get_simplex_up("
              << casc::to_string(mesh.get_name(vid)) << ")), " << std::boolalpha
              << mesh.onBoundary(vid) << ");" << std::endl;
    std::cout << "EXPECT_EQ(mesh.nearBoundary(mesh.get_simplex_up("
              << casc::to_string(mesh.get_name(vid)) << ")), " << std::boolalpha
              << mesh.nearBoundary(vid) << ");" << std::endl;
  }
  ...
  for (auto eid : mesh.get_level_id<2>()) {
    ...
    std::cout << std::endl;
    std::cout << "EXPECT_EQ(mesh.onBoundary(mesh.get_simplex_up("
              << casc::to_string(mesh.get_name(eid)) << ")), " << std::boolalpha
              << mesh.onBoundary(eid) << ");" << std::endl;
    std::cout << "EXPECT_EQ(mesh.nearBoundary(mesh.get_simplex_up("
              << casc::to_string(mesh.get_name(eid)) << ")), " << std::boolalpha
              << mesh.nearBoundary(eid) << ");" << std::endl;
  }

  for (auto fid : mesh.get_level_id<3>()) {

    std::cout << "EXPECT_EQ(mesh.onBoundary(mesh.get_simplex_up("
              << casc::to_string(mesh.get_name(fid)) << ")), " << std::boolalpha
              << mesh.onBoundary(fid) << ");" << std::endl;
    std::cout << "EXPECT_EQ(mesh.nearBoundary(mesh.get_simplex_up("
              << casc::to_string(mesh.get_name(fid)) << ")), " << std::boolalpha
              << mesh.nearBoundary(fid) << ");" << std::endl;
  }
